# record-platform-stack — P1_AZURE_REACT_DOTNETCORE_CLI_CPM_MVC_MONO
...
> **Build tool note:** Built with the plain `dotnet build`/`dotnet run` CLI workflow, with the SDK version pinned via `global.json`.
...
- **CI**: Azure DevOps (`azure-pipelines.yml`), building the API with this
  branch's own build tool (.NET CLI) before building the client.
...
dotnet build RecordsApi/RecordsApi.csproj -c Release
