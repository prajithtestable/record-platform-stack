# Digital Sippoy — DIGITAL_SIPPOY_JS_V20_VITE_BUN_MICRO
...
> **Bundler substitution note:** Next.js's own default build tool is used instead (Webpack, the `next build` default in Next.js 15.5.12). Vite is not a pluggable bundler for `next build` either — Next.js has its own separate build pipeline unrelated to Vite's. This branch documents the substitution honestly, the same way the Vite-labeled branches in the record-platform-stack (JS_V22_*) matrix do for Angular.
...
Or locally with bun (Node 20 required):

```bash
bun install --frozen-lockfile
bun run dev      # dev server
bun run build    # production build
bun run start      # after building
```
