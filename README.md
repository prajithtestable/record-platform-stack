# record-platform-stack — P1_AZURE_REACT_DOTNETCORE_CAKE_CPM_MVC_MONO
...
> **Build tool note:** Driven by `RecordsApi/build.cake`, a Cake (C# Make) script, via the local `Cake.Tool` .NET tool declared in `.config/dotnet-tools.json`. Bootstrapping the tool (`dotnet tool restore`) requires nuget.org access and was **not** verified in this template's sandbox.

> **Package manager note:** Package versions are centralized in a root `Directory.Packages.props` (`ManagePackageVersionsCentrally=true`); `RecordsApi.csproj` lists `<PackageReference>` items with no `Version` attribute.

> **Architecture-note caveat:** like the rest of this corpus, "Monolith" is a
...
- **CI**: Azure DevOps (`azure-pipelines.yml`), building the API with this
  branch's own build tool (Cake) before building the client.
...
dotnet cake RecordsApi/build.cake --target=Build
