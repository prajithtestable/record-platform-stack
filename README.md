# record-platform-stack — JS_V22_VITE_PNPM_MICRO
...
> **Bundler substitution note:** esbuild is used in its place. Vite is **not** a real standalone Angular 20 CLI bundler — Angular's esbuild Application Builder uses Vite internally only for its dev-server (`ng serve`), never for `ng build`. There is no `@angular-builders` or first-party config that swaps Vite in as the production bundler, so this branch honestly ships the esbuild Application Builder and documents the substitution here rather than silently mislabeling esbuild output as "Vite".

> **Architecture-note caveat:** like the rest of this matrix, "Microservices" is a
...
Or run each piece locally with pnpm:

```bash
# backend-service-a
cd backend-service-a && pnpm install --frozen-lockfile && pnpm start

# backend-service-b (needs Elasticsearch + LocalStack reachable)
cd backend-service-b && pnpm install --frozen-lockfile && pnpm start

# frontend
cd frontend && pnpm install --frozen-lockfile && pnpm start
```

Production build of the frontend: `pnpm build` (run inside `frontend/`).
